...
def handler(input_data, context):
    ...
    try:
        ...
        for c in candidates:
            ev = _evidence_for(c)
            res = score_candidate(
                c.get("candidate_id", "cand_unknown"),
                EvidenceScores(**{k: ev.get(k, 0.0) for k in _EVIDENCE_KEYS}),
            )
            ranked.append(
                {
        ...
        W = {
            ...
            "handle_watermark": 0.15,
            "ocr_caption_query": 0.15,
            "predates_input": 0.15,
            ...
            ranked.append(
                {
                    ...
                    "evidence": ev,
                    "caveats": [
                        "Telegram post may itself be a repost.",
