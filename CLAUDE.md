...
- **API-token 403 on execute/chat is a permission-key *format* issue, not an ACL (#44).** The runtime requires a **namespaced** key (`sinas.functions/<ns>/<name>.execute:own`, `sinas.agents/<ns>/<name>.chat:own`); the **flat** `sinas.functions.execute:all` does **not** match it. The console works because the JWT session loads the role's `sinas.*:all`; an API token carries only its fixed permission subset. Fix: give the key namespaced perms (or `sinas.*:all`) — the installer owns the resources so `:own` suffices. Confirm with `POST /auth/check-permissions` on the *namespaced* string (returns `false`). See [docs/research/runtime-diagnostics.md](docs/research/runtime-diagnostics.md).
