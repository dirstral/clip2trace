...
- **API-token 403 on execute/chat is a *token* restriction, not an ACL or format bug (#44).** Measured on via-10: the correct permission-key grammar is `sinas.<resource>.<action>:<scope>` (e.g. `sinas.functions.execute:own`); namespace is metadata, not part of the key. The existing API token has **explicit per-key overrides** (`APIKeyCreate.permissions`) that strip every write/admin capability — the user is in `Admins` but the token inherits only what its overrides grant. The console works because it uses the user's JWT session, not this token. Fix: mint a new key with `permissions: {}` (inherit) or with explicit least-privilege perms. Diagnose with `python3 scripts/diagnose_permissions.py --execute clip2trace/diagnose_runtime`. Full evidence in [docs/research/permissions-diagnosis.md](docs/research/permissions-diagnosis.md) (the PR #45 format claim was wrong; see the "CORRECTION" note in [docs/research/runtime-diagnostics.md](docs/research/runtime-diagnostics.md)).
