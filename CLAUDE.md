...
- **Commit messages follow [Conventional Commits 1.0.0](https://www.conventionalcommits.org/en/v1.0.0/):** `type(scope): description` (types: `feat`, `fix`, `docs`, `test`, `refactor`, `chore`, `build`, `ci`, `perf`, `style`; `!` or a `BREAKING CHANGE:` footer for breaks). Lowercase the description, keep the subject ≤ ~72 chars, put detail in the body. **Do not add any AI / `Co-Authored-By` self-attribution trailer.**
