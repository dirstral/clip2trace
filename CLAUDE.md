...
- **API keys CANNOT execute/read package resources on via-10 — platform bug, not our config (#44).** Confirmed 2026-06-03: a freshly minted key for the same admin user with explicit `sinas.functions.execute:all` + `sinas.functions.read:all` (the create response echoes them all `true`) **still** gets `403 "Not authorized to execute this function"`, `403 "Not authorized to read this resource"`, and `GET /api/v1/functions` → `[]` — **while** `/auth/check-permissions` returns `has_permission: true` for those same keys. So the permission-check layer honors key permissions but the **resource-authorization middleware ignores them**; an API-key principal never clears the per-resource gate. The console works only because it uses the user's **JWT session** (auth resolves via the `Admins` role). **Workaround: authenticate with a JWT, not an API key** — `POST /auth/login` → use `access_token` as bearer; refresh via `POST /auth/refresh {refresh_token}` (~15-min TTL). **Real fix is operator/platform-side.** Earlier diagnoses were wrong: PR #45 blamed the key *format* (`sinas.<resource>.<action>:<scope>` is correct, namespace is metadata), and the first correction blamed empty/stripped key permission overrides + "mint a key with explicit perms" — both disproven. Full evidence + reproducer in [docs/research/permissions-diagnosis.md](docs/research/permissions-diagnosis.md) (`scripts/diagnose_permissions.py`).
