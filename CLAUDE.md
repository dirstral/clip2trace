...
- **API keys CAN execute — grant *wildcard* permissions, not explicit actions (#44, RESOLVED 2026-06-03).** Mint the key with `sinas.<resource>.*:all` (or `sinas.*:all`), **not** explicit `sinas.functions.execute:all`. Measured on via-10 (same admin user, real keys): `{"sinas.functions.*:all": true, ...}` → execute **200**; `{"sinas.functions.execute:all": true, ...}` → **403**. The catch: `/auth/check-permissions` matches explicit action keys (returns `true`) but the **resource-auth middleware only matches wildcards**, so an explicit-perms key *looks* correct yet silently 403s — that mismatch is the remaining platform bug worth filing. Least-privilege working key: grant wildcard perms (each `= true`) for `functions`, `agents`, `collections`, `stores`, `executions`, `components` — i.e. `sinas.<resource>.*:all` for those six (avoid `sinas.*:all` = full admin); the exact copy-pasteable `POST /api/v1/api-keys` body is in the diagnosis doc. Verify with `python3 scripts/diagnose_permissions.py --token <key> --execute clip2trace/diagnose_runtime`. Full recipe + evidence in [docs/research/permissions-diagnosis.md](docs/research/permissions-diagnosis.md); operator write-up in [docs/research/sinas-api-key-execute-bug-report.md](docs/research/sinas-api-key-execute-bug-report.md). (Earlier diagnoses — PR #45's key-format claim, and the "can't execute / use JWT" reading — were wrong: they only tested explicit-action keys.)
