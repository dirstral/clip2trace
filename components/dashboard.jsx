...
// The coordinator persists job status to the clip2trace/jobs store (key = job_id)
// as it advances created -> analyzing -> ... -> done; the component has readonly
// access, so this is the richest progress signal. Best-effort: returns null if
// the store API isn't reachable from this embed context.
async function readJobState(sinas, jobId) {
  const stores = (sinas && sinas.stores) || {};
  try {
    if (typeof stores.get === "function") {
      return await stores.get("clip2trace/jobs", jobId);
    }
    if (typeof stores.getState === "function") {
      return await stores.getState({ store: "clip2trace/jobs", key: jobId });
    }
  } catch (e) {
    return null;
  }
...
export default function Dashboard({ sinas }) {
  ...
  const runDemo = useCallback(() => {
    ...
    setStage("progress");
    // status -> progress mirrors clip2trace.storage.JOB_PROGRESS.
    const steps = [
      ["analyzing", 0.15],
      ["searching", 0.4],
      ["verifying", 0.6],
    ...
    pollRef.current = setInterval(() => {
      i += 1;
      const [s, progress] = steps[Math.min(i, steps.length - 1)];
      setStatus(s);
      setJob((j) => ({ ...(j || {}), status: s, progress }));
      if (s === "done") {
        stopPolling();
  ...
  // Live/hybrid (#20): create the job, then run the LONG analysis step
  // (analyze_input_video) via async execution — it can exceed the 300 s sync
  // timeout once it downloads + decodes a real upload. Poll the execution for
  // terminal state and the jobs store for fine-grained status; surface failures.
  const runLive = useCallback(async () => {
    resetRun();
    setStatus("creating");
    try {
      const created = await callFunction(sinas, "clip2trace/create_job", { mode });
      // create_job's output may be top-level or wrapped under `result`.
      const createdData = (created && created.result) || created || {};
      const jobId = created.job_id || createdData.job_id;
      const fileId = created.input_video_file_id || createdData.input_video_file_id;
      setJob({ job_id: jobId, status: "created", mode });
      setStage("progress");

      setStatus("analyzing");
      const enq = await enqueueFunction(sinas, "clip2trace/analyze_input_video", {
        job_id: jobId,
        input_video_file_id: fileId,
        mode,
      });

      // When the analysis finishes, fetch the report for the segment/candidate view.
      // Returns true on success, false if it surfaced an error (so the poller stops).
      const finish = async (analysisRaw) => {
        // Functions signal failure by returning {error: ...} rather than throwing.
        const analysis = (analysisRaw && analysisRaw.result) || analysisRaw || {};
        if (analysis.error) {
          setError(`analysis failed: ${analysis.error}`);
          setStatus("failed");
          return false;
        }
        // A degraded run (real upload that fell back to demo/uniform segments) is
        // surfaced as a warning without failing the whole job (#20).
        if (analysis.degraded) {
          setWarning(analysis.warning || "analysis ran in degraded mode");
        }
        setJob((j) => ({ ...(j || {}), status: "reporting" }));
        const rep = await callFunction(sinas, "clip2trace/render_report", {
          job_id: jobId,
          segments: analysis.segments || [],
          mode,
        });
        const repData = (rep && rep.result) || rep;
        if (repData && repData.error) {
          setError(`report failed: ${repData.error}`);
          setStatus("failed");
          return false;
        }
        setReport(repData);
      ...
      if (enq.execution_id) {
        ...
        stopPolling();
        // Each tick awaits two network calls (jobs store + executions); guard so a
        // slow tick can't overlap the next one and race state updates.
        let inFlight = false;
        pollRef.current = setInterval(async () => {
          if (inFlight) return;
          inFlight = true;
          try {
            // Prefer the jobs-store status (coordinator-updated) for the label.
            const jobState = await readJobState(sinas, jobId);
            const ex = await getExecution(sinas, enq.execution_id);
            setStatus((jobState && jobState.status) || ex.status || "running");
            if (jobState && typeof jobState.progress === "number") {
              setJob((j) => ({ ...(j || {}), progress: jobState.progress }));
            }
            if (ex.status === "completed" || ex.status === "succeeded") {
              stopPolling();
              await finish(ex.result || ex.output);
            } else if (ex.status === "failed" || ex.status === "error") {
              stopPolling();
              setError(ex.error || "analysis execution failed");
              setStatus("failed");
            }
          } catch (e) {
            stopPolling();
        ...
        // Synchronous fallback (short jobs / clients without async support).
        await finish(enq.result);
      }
    } catch (e) {
      setError(String((e && e.message) || e));
      setStatus("failed");
  ...
  return (
    <div style={{ fontFamily: "system-ui", padding: 16, maxWidth: 920 }}>
      ...
      {warning && (
        <div style={{ background: "#8d6e00", color: "#fff", padding: 10, borderRadius: 6 }}>
          Warning: {warning}
        </div>
      )}
      ...
      {stage === "progress" && (
        <section>
          ...
          <p>
            ...
            {job && typeof job.progress === "number"
              ? ` · ${Math.round(job.progress * 100)}%`
              : ""}{" "}
            {pollRef.current ? "(polling…)" : ""}
          </p>
          <p style={{ color: "#777", fontSize: 13 }}>
            The long analysis step (<code>analyze_input_video</code>) runs async
            (execute/async → poll /executions/&#123;id&#125; + the clip2trace/jobs store);
            failures surface above.
          </p>
        </section>
      )}
